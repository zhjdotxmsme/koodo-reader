...
/**
 * Read-only access to one desktop-format `.db` file (any database the
 * desktop engine keeps: `<table>.db` or its `temp-<table>.db` scratch copy —
 * both contain the same-named table).
 */
class DesktopDbReader(private val file: File) : AutoCloseable {
...
/**
 * Writer for desktop-format `.db` files (native → desktop direction).
 * Creates the table with the EXACT desktop DDL (schema.lock) so the desktop
 * engine reads the file back without surprises.
 */
class DesktopDbWriter(private val file: File) : AutoCloseable {
